...
make_step <- function(step_id, frq_source, ini_source, tag_source, age_source,
  ...
  write_doitall(
    ...
    reviewed_selectivity = isTRUE(doitall_edits$reviewed_selectivity),
  ...
  entries <- list(
    ...
    list(role = "doitall", file = "doitall.sh", source = file.path("steps", template_step_id, "model", "doitall.sh"), note = paste(c(
      ...
      if (isTRUE(doitall_edits$reviewed_selectivity)) "reviewed fishery-level selectivity controls applied",
